...
function validateCounts(counts: number[]): void {
  if (
    ...
    counts.some((n) => !Number.isInteger(n) || n < 0 || n > 8) ||
...
export function parseRule(text: string): Rule {
  ...
  if (!match)
    throw new Error("Use B…/S… notation with neighbor counts from 0 to 8.");
  const rule = {
    birth: [...match[1]].map(Number),
    survival: [...match[2]].map(Number),
  };
...
export function step(
  ...
  if (
    ...
    throw new Error("Grid must contain one binary value per cell.");
  if (boundary !== "wrap" && boundary !== "dead")
    throw new Error("Unknown boundary mode.");
  const birth = new Uint8Array(9);
  const survival = new Uint8Array(9);
  rule.birth.forEach((n) => {
    birth[n] = 1;
  });
  rule.survival.forEach((n) => {
    survival[n] = 1;
  });
    ...
    for (let x = 0; x < width; x++) {
      ...
      if (noiseProbability > 0) {
        ...
        if (!Number.isFinite(sample) || sample < 0 || sample >= 1)
          throw new Error("Random values must be in [0, 1).");
...
export function metrics(
  before: Uint8Array,
  after: Uint8Array,
): { density: number; activity: number } {
  if (before.length === 0 || before.length !== after.length)
    throw new Error("Measurements require equal, nonempty grids.");
  let live = 0;
  let changed = 0;
  for (let i = 0; i < after.length; i++) {
    if (before[i] > 1 || after[i] > 1)
      throw new Error("Measurements require binary grids.");
    live += after[i];
...
export function patternGrid(
  ...
  if (width < patternWidth || height < patternHeight)
    throw new Error("Pattern does not fit the grid.");
