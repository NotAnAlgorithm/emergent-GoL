...
describe("rules", () => {
  ...
  it.each(["B9/S23", "B33/S23", "B3/S22", "23/3", "B-1/S", "B3/S2x"])(
    "rejects %s",
    (text) => {
  ...
  it("evaluates all birth and survival neighbor counts", () => {
    ...
    for (let count = 0; count <= 8; count++) {
      for (const alive of [0, 1]) {
        ...
        neighbors.slice(0, count).forEach((i) => {
          grid[i] = 1;
        });
        grid[4] = alive;
        const matching = alive
          ? { birth: [], survival: [count] }
          : { birth: [count], survival: [] };
        const opposite = alive
          ? { birth: [count], survival: [] }
          : { birth: [], survival: [count] };
        expect(step(grid, 3, 3, matching, "dead")[4]).toBe(1);
        expect(step(grid, 3, 3, opposite, "dead")[4]).toBe(0);
        expect(step(grid, 3, 3, { birth: [], survival: [] }, "dead")[4]).toBe(
          0,
        );
      }
    }
...
describe("synchronous evolution", () => {
  ...
  it("rotates a blinker then restores it", () => {
    ...
    expect(
      [...next.entries()].filter(([, cell]) => cell).map(([i]) => i),
    ).toEqual([7, 12, 17]);
  ...
  it("counts eight neighborhood positions on a one-cell torus", () => {
    expect(step(new Uint8Array([1]), 1, 1, parseRule("B/S8"), "wrap")[0]).toBe(
      1,
    );
  });
  it("revives an empty B0 grid", () => {
    expect(step(new Uint8Array(16), 4, 4, parseRule("B0/S"), "dead")).toEqual(
        ...
        for (const notation of ["B3/S23", "B0368/S1247", "B/S012345678"]) {
          ...
          const expected = grid.map((alive, index) => {
            ...
            const count = offsets.reduce((sum, [dx, dy]) => {
              ...
              return (
                sum +
                grid[((ny + height) % height) * width + ((nx + width) % width)]
              );
            }, 0);
...
describe("randomness and measurements", () => {
  it("replays random grids and keeps probabilities bounded", () => {
    ...
    expect(randomGrid(20, 20, 0.3, 42)).not.toEqual(
      randomGrid(20, 20, 0.3, 43),
    );
  ...
  it("applies noise after the update and replays its stream", () => {
    ...
    expect(
      step(grid, 12, 12, life, "wrap", 0, () => {
        throw new Error("unused");
      }),
    ).toEqual(clean);
    expect(step(grid, 12, 12, life, "wrap", 1, createRng(5))).toEqual(
      clean.map((cell) => cell ^ 1),
    );
    const run = () => {
      let current = grid;
      const rng = createRng(99);
      for (let i = 0; i < 10; i++)
        current = step(current, 12, 12, life, "wrap", 0.1, rng);
      return current;
    };
  ...
  it("reports live density and changed-cell fraction", () => {
    expect(
      metrics(new Uint8Array([0, 0, 1, 1]), new Uint8Array([0, 1, 0, 0])),
    ).toEqual({ density: 0.25, activity: 0.75 });
  });
  it("rejects invalid inputs", () => {
    ...
    expect(() =>
      step(new Uint8Array(4), 2, 2, life, "dead", 1, () => 1),
    ).toThrow();
