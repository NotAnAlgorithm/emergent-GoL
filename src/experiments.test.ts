...
import { aggregateResults, createRecord, exportRecord, importRecord, measurementsCsv, noiseConfigs, runExperiment, sampleRules, surveyConfigs, type ExperimentConfig } from './experiments';

const config: ExperimentConfig = { rule: 'B3/S23', width: 8, height: 8, boundary: 'wrap', density: 0.3, initialSeed: 42, noiseSeed: 99, noiseProbability: 0.01, generations: 12 };
describe('experiment execution', () => {
  it('replays noisy trajectories exactly with independent noise and initial seeds', () => {
    ...
    expect(runExperiment({ ...config, noiseSeed: 2 }).initialGrid).toEqual(runExperiment(config).initialGrid);
  });
  it('agrees with interactive engine stepping', () => {
    let grid = randomGrid(8, 8, 0.3, 42);
    const rng = createRng(99);
    for (let i = 0; i < 12; i++) grid = step(grid, 8, 8, parseRule(config.rule), 'wrap', 0.01, rng);
    expect(runExperiment(config).finalGrid).toEqual(Array.from(grid));
  });
  it('summarizes only the final 500 generations', () => {
    const result = runExperiment({ ...config, width: 3, height: 3, generations: 520 });
    const tail = result.measurements.slice(20);
    expect(result.summary.density).toBe(tail.reduce((sum, m) => sum + m.density, 0) / 500);
    expect(result.summary.activity).toBe(tail.reduce((sum, m) => sum + m.activity, 0) / 500);
  });
  it('continues past empty grids under B0', () => {
    const result = runExperiment({ ...config, rule: 'B0/S', width: 3, height: 3, noiseProbability: 0, generations: 1 }, Array(9).fill(0));
...
describe('study design', () => {
  ...
  it('uses nine matched starting conditions across 100 surveyed rules', () => {
    const configs = surveyConfigs({ width: 8, height: 8, generations: 3, samplingSeed: 5 });
    expect(configs).toHaveLength(900);
    expect(new Set(configs.map(c => c.rule)).size).toBe(100);
    expect(configs.slice(0, 9).map(({ rule: _rule, ...c }) => c)).toEqual(configs.slice(9, 18).map(({ rule: _rule, ...c }) => c));
  });
...
describe('experiment files', () => {
  it('round-trips exact initial states and reproduces the run', () => {
    ...
    const saved = createRecord(config, result.initialGrid, 'Trial', 'Observation');
    const imported = importRecord(exportRecord(saved));
    expect(imported).toEqual(saved);
    expect(runExperiment(imported.config, imported.initialGrid)).toEqual(result);
  ...
  it('rejects unsupported versions, invalid cells, and invalid metadata', () => {
    ...
    expect(() => importRecord(JSON.stringify({ ...saved, version: 2 }))).toThrow();
    expect(() => importRecord(JSON.stringify({ ...saved, initialGrid: Array(64).fill(2) }))).toThrow();
    expect(() => importRecord(JSON.stringify({ ...saved, notes: null }))).toThrow();
    expect(() => importRecord('{')).toThrow();
  });
});

describe('notebook validation', () => {
  const png = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+a5XcAAAAASUVORK5CYII=';
  it('uses UI record fields and preserves screenshots and generation', () => {
    ...
    expect(createRecord(config, Array(64).fill(0), 'Test').generation).toBe(config.generations);
  });
  it('rejects invalid image URLs and generation values', () => {
    const record = createRecord(config, Array(64).fill(0), 'Test');
    for (const screenshot of ['https://example.com/image.png', 'data:image/svg+xml;base64,AAAA', 'data:image/png;base64,AAAA', png + '!']) {
      expect(() => importRecord(JSON.stringify({ ...record, screenshot }))).toThrow(/PNG/);
    }
    for (const generation of [-1, 0.5, 4294967296, null]) {
      expect(() => importRecord(JSON.stringify({ ...record, generation }))).toThrow(/Generation/);
    }
  });
  it('validates uint32 seeds and allows general grid dimensions', () => {
    expect(() => runExperiment({ ...config, initialSeed: 4294967295, noiseSeed: 0 })).not.toThrow();
    for (const seed of [-1, 4294967296, 0.5]) {
      expect(() => runExperiment({ ...config, initialSeed: seed })).toThrow(/32-bit/);
      expect(() => runExperiment({ ...config, noiseSeed: seed })).toThrow(/32-bit/);
    }
    const small = { ...config, width: 3, height: 7 };
    expect(importRecord(exportRecord(createRecord(small, Array(21).fill(0), 'Rectangle'))).config).toEqual(small);
  });
  it('rejects records larger than 5 MB, counting UTF-8 bytes', () => {
    const record = createRecord(config, Array(64).fill(0), 'Test');
    expect(() => importRecord(' '.repeat(5 * 1024 * 1024 + 1))).toThrow(/5 MB/);
    expect(() => importRecord(JSON.stringify({ ...record, notes: 'é'.repeat(3 * 1024 * 1024) }))).toThrow(/5 MB/);
  });
});

describe('noise comparisons and replicate summaries', () => {
  it('measures divergence from a matched noiseless trajectory', () => {
    const noisy = runExperiment({ ...config, width: 3, height: 3, noiseProbability: 1, generations: 1 }, Array(9).fill(0));
  ...
  it('compares evolving baseline grids at every generation', () => {
    ...
    for (let i = 0; i < config.generations; i++) {
      baseline = step(baseline, config.width, config.height, parseRule(config.rule), config.boundary);
      actual = step(actual, config.width, config.height, parseRule(config.rule), config.boundary, config.noiseProbability, rng);
      const difference = baseline.reduce((sum, cell, index) => sum + Number(cell !== actual[index]), 0) / baseline.length;
  ...
  it('groups rules, densities, and noise levels and uses sample standard deviation', () => {
    ...
    const results = [0.2, 0.4, 0.6].map(value => ({ ...base, summary: { density: value, activity: value / 2, difference: value } }));
